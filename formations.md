---
title: "Formations"
order: 1
in_menu: true
blog_index: true
---
# Catalogue de stages 2025-2026
 
Tous ces stages ont déjà eu lieu en 2025 ou précédemment. Les stages à
distance peuvent être organisés en présentiel. Je suis à votre écoute
pour adapter ces formations prêtes à l'emploi en fonction de vos
souhaits et de vos objectifs.

## [Nouveau] Recherche documentaire et rédaction bibliographique - École des Plantes de Paris

-   Durée : 2 x 1h
-   Modalité : à distance
-   [Support de formation et
    descriptif](https://github.com/fflamerie/biblio_EDPP)

## [Nouveau] Gestion et rédaction bibliographiques avec l'IA

-   Durée : 3h
-   Modalité : à distance
-   [Support de formation et
    descriptif](https://github.com/fflamerie/biblio_IA/)

## Gérer facilement sa bibliographie avec Zotero

-   Durée : 3h
-   Modalité : à distance
-   [Support de formation et
    descriptif](https://github.com/fflamerie/zotero_gerer_biblio)

## Reference management made easy with Zotero

-   Durée : 3h
-   Modalité : à distance
-   [Support de formation et
    descriptif](https://github.com/fflamerie/zotero_easy)

## Zotero pour les bibliothécaires

-   Durée : 3h
-   Modalité : à distance
-   [Support de formation et descriptif
    ](https://github.com/fflamerie/zotero_bibpro_dist)

## Approfondissement Zotero : les extensions de Zotero

-   Durée : 3h
-   Modalité : à distance
-   [Support de formation et descriptif
    ](https://github.com/fflamerie/zotero_modules)

Je propose également une version de ce stage destinée spécifiquement aux
professionnels de la documentation, à partir du même support de
formation.

## Zotero pour les revues de littérature

-   Durée : 3h
-   Modalité : à distance et en présentiel
-   [Support de formation et
    descriptif](https://github.com/fflamerie/zotero_revlit)

## Revues de littérature : enjeux, méthodes et outils bibliographiques

-   Durée : 4h
-   Modalité : en présentiel et à distance
-   [Support de formation et
    descriptif](https://github.com/fflamerie/intro_revlit)

## Zotero formation de formateurs

-   Durée : 5h
-   Modalité : en présentiel
-   [Support de formation et descriptif
    ](https://github.com/fflamerie/zotero_formateurs)

## Créer des styles bibliographiques CSL

-   Durée : à déterminer
-   Modalité : à déterminer
-   Support de formation et descriptif : [stage d'une
    journée](https://github.com/fflamerie/zotero_csl) pour la version
    longue et l'[atelier
    d'1h30](https://github.com/fflamerie/zotero_csl_libreABC) pour la
    version courte, avec de forts pré-requis techniques. 